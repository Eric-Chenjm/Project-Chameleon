...
def apply_string_translation(content, original, translated):
    literal_pattern = re.compile(r"""^(["']).*\1$""")
    # 1. 显式带有外层引号的字面量（例如 '"New Conversation"'）
    if literal_pattern.match(original) and literal_pattern.match(translated):
        return apply_safe_literal_replacement(content, original, translated)

    # 2. 显式带有冒号或属性键的代码片段（例如 label:"New Conversation" 或 title:"Account"）
    if any(k in original for k in ['label:', 'title:', 'aria-label:']):
        if original in content:
            return content.replace(original, translated), True
        alt_o = original.replace('"', "'")
        alt_t = translated.replace('"', "'")
        if alt_o in content:
            return content.replace(alt_o, alt_t), True
        return content, False

    # 3. 严格字面量安全替换：必须匹配被引号包裹的形式，严禁做裸词子串替换
    src_quoted_d = f'"{original}"'
    dst_quoted_d = f'"{translated}"'
    src_quoted_s = f"'{original}'"
    dst_quoted_s = f"'{translated}'"

    src_escaped = original.replace('\\', '\\\\').replace('"', '\\"')
    dst_escaped = translated.replace('\\', '\\\\').replace('"', '\\"')
    src_escaped_d = f'"{src_escaped}"'
    dst_escaped_d = f'"{dst_escaped}"'

    replaced = False
    if src_quoted_d in content:
        content = content.replace(src_quoted_d, dst_quoted_d)
        replaced = True
    if src_quoted_s in content:
        content = content.replace(src_quoted_s, dst_quoted_s)
        replaced = True
    if src_escaped_d != src_quoted_d and src_escaped_d in content:
        content = content.replace(src_escaped_d, dst_escaped_d)
        replaced = True

    # 4. 对于完整的多词长句（长度 >= 25 且必须包含空格），允许在纯文本模板中替换
    if not replaced and len(original) >= 25 and (' ' in original):
        if original in content:
...
def main():
    ...
    translations = {
        "Failed with exit code": "进程退出，错误码：",
        "Active Terminal": "活动终端",
        "Send to Terminal": "发送到终端",
        "Relaunch Terminal": "重启终端",
        "Kill Terminal": "终止终端",
        "Background color of inline hints for types": "类型行内提示的背景颜色",
        "Foreground color of inline hints": "行内提示的前景色",
        "Background color of inline hints for parameters": "参数行内提示的背景颜色",
        "Failed to fetch turn diff:": "获取轮次代码差异失败：",
        "Failed to reject browser interaction": "拒绝浏览器交互失败",
        "Failed to reject execution": "拒绝执行操作失败",
        "Failed to load diff contents.": "加载代码差异内容失败。",
        "Agent will always ask to review in strict mode.": "在严格模式下，智能体执行操作前将始终请求评审。",
        "Agent terminated due to error": "智能体因错误异常终止",
        "Agent sound playback blocked.": "智能体提示音播放被阻止。",
        ...
        "Diff contents are no longer available.": "Diff 差异内容已失效或不再可用。",
    "Failed to load diff contents.": "加载 Diff 差异内容失败。",
    "Failed to load diff.": "加载代码差异失败。",
    "Failed to load some diffs": "部分代码差异加载失败",
    "Failed to fetch diffs": "获取代码差异失败",
    "Failed to fetch turn diff:": "获取本轮对话代码差异失败：",
    "Files modified by the agent in this conversation": "智能体在本轮对话中修改的文件",
    "Cannot revert messages with file diff comments": "包含文件差异批注的消息无法撤销",
    "Diff overview ruler foreground for inserted content.": "Diff 概览标尺中新增内容的前景色。",
    "Diff overview ruler foreground for removed content.": "Diff 概览标尺中删除内容的前景色。",
    "Diff worker fatal error:": "Diff 后台工作线程发生致命错误：",
    "Toggle Diff": "切换差异视图",
    "Diff View": "Diff 视图",
    "Modified Files": "已修改文件",
    "Modified files:": "已修改文件：",
    "Modified in": "修改于",
    "Added line count did not match for hunk at line": "代码块新增行数在行首不匹配：",
    "Removed line count did not match for hunk at line": "代码块删除行数在行首不匹配：",
    ...
    "Paste auth code": "粘贴授权凭据",
    "Rename": "重命名",
    "Renamed": "已重命名",
    "Delete Permanently": "永久删除",
    "Collapse All": "全部折叠",
    "Duplicate project name": "项目名称重复",
    "Failed to rename conversation": "重命名会话失败",
    "Execute a command": "执行终端命令",
    "Execute an MCP tool call": "执行 MCP 工具调用",
    "Stop Execution": "停止执行",
    "Stop execution": "停止执行",
    "Keyboard shortcuts": "键盘快捷键",
    "Open Keyboard Shortcuts": "打开键盘快捷键",
    "Keyboard shortcuts for quick navigation and control.": "用于快速导航与全局控制的键盘快捷键。",
    ...
    "No background tasks": "暂无运行中的后台任务",
    "Process exited with an error": "进程异常退出",
    "Error while running command": "执行命令时发生错误",
    "Process completed with exit code": "进程执行完毕，退出代码：",
    "Start Snapshot": "起始快照",
    "End Snapshot": "完成状态快照",
    "Select the latest snapshot from the": "从列表中选择最新的工作区快照：",
    "Staged index changes and working tree changes": "Git 暂存区更改与工作区未暂存更改",
    "Discard Workspace Changes": "放弃工作区更改",
    "Enable Sandbox Mode": "启用沙箱模式",
    "Enable Sandbox Mode (Preview)": "启用沙箱模式 (预览版)",
    "Enable Terminal Sandbox": "启用终端沙箱隔离",
    "Sandboxed": "沙箱受限模式 (推荐安全实践)",
    "BypassSandbox": "绕过沙箱",
    "Network Access Rules": "网络访问规则",
    "Restricts agent tools to a secure, isolated local sandbox.": "将智能体工具限制在安全、隔离的本地沙箱环境中运行。",
    "Run terminal commands with sandbox restrictions.": "在沙箱安全策略限制下运行终端命令。",
    "Allow sandboxed commands to make network requests.": "允许沙箱内的命令发起网络请求。",
    "Allow/deny agent command execution outside the sandbox.": "允许/禁止智能体在沙箱外部执行命令。",
    "Commands the agent can run outside the sandbox.": "允许智能体在沙箱外部运行的命令白名单。",
    "Configure allowed commands outside the sandbox.": "配置允许在沙箱外部执行的命令白名单。",
    "Failed to add URL to allowlist:": "将网址添加至白名单失败：",
    "Upstream proxy returned an empty response": "上游代理服务器返回了空响应",
    "Allow full browser script execution without prompting.": "允许全量执行浏览器脚本且无需提示确认。",
    "Block all browser JavaScript execution.": "禁止所有浏览器端 JavaScript 脚本执行。",
    "Browser Javascript Execution Policy": "浏览器端 JavaScript 执行策略",
    "Application": "应用偏好设置",
    "Skin": "界面形态与风格",
    "Tab": "Tab 智能补全与导航",
    "Browser": "内置浏览器与执行规则",
    "Customizations": "自定义扩展 (技能/MCP/插件)",
    "Shortcuts": "键盘快捷键速查",
    "Workspace Settings": "工作区专属设置",
    "Jetski Chat": "内部调试会话 (Jetski)",
    "Regroup Google3 Chats": "重组 Google3 对话",
    "Provide Feedback": "意见与问题反馈",
    "Open Settings": "打开设置",
    "Close Settings": "关闭设置",
    "Open Preferences": "打开偏好设置",
    "Open System Preferences": "打开系统偏好设置",
    "Open Editor Settings": "打开底层编辑器核心设置",
    "Settings Unavailable": "设置面板暂时不可用",
    "Agent Settings (For Project)": "智能体现有项目专属配置",
    "Agent Security Settings": "智能体安全策略与权限配置",
    "Skills and Rules Settings": "技能与提示词规则设置",
    "Theme": "界面主题",
    "Light Theme": "浅色主题配色",
    "Dark Theme": "深色主题配色",
    "Inherit Editor": "继承编辑器主题",
    "Contrast": "界面对比度",
    "Strong": "高对比度增强",
    "Preset": "配色预设方案",
    ...
    "Dracula": "Dracula (德古拉暗黑)",
    "Vesper": "Vesper (暮色微光)",
    "Catppuccin": "Catppuccin (马卡龙)",
    "Monokai": "Monokai (经典暗色)",
    "One Dark Pro": "One Dark Pro (Atom 暗色)",
    "One Light": "One Light (Atom 浅色)",
    "Solarized Dark": "Solarized Dark (日光深色)",
    "Solarized Light": "Solarized Light (日光浅色)",
    "Configure the agent": "配置智能体的外观与交互视觉表现",
    "Choose the product skin that fits how you work.": "选择契合您工作习惯的产品交互界面形态。",
    "Workspace settings have moved": "工作区设置已迁移",
    "Best of N settings have moved": "多候选探索设置已迁移",
    "Browser settings have moved": "浏览器设置已迁移",
    "Labs settings have moved": "实验特性设置已迁移",
    "Change VCS in General settings, under Advanced": "请在【通用设置】的【高级】选项中切换版本控制系统 (VCS)",
    "Use full Git clones for Best of N": "在多候选探索中对 Git 仓库使用完整独立克隆",
    "Use full VCS clones for Best of N": "在多候选探索中对 VCS 仓库使用完整独立克隆",
    "Security Preset": "安全策略预设",
    "Full access": "完全访问模式 (整机与网络权限开放)",
    "Full machine": "整机完全控制模式",
    "Turbo": "Turbo 极速狂飙模式",
    "Turbo mode": "Turbo 极速自动模式",
    "Workspace only": "仅限工作区范围",
    "Always ask": "始终弹窗审批确认",
    "Global Permissions": "全局安全权限",
    "Review Permissions": "审核并授予权限",
    "automation. Automations do not inherit Global Permissions.": "自动化运行任务不会继承全局权限，需单独配置安全授权。",
    "Browser Actuation Permissions": "浏览器自动化操作与页面控制权限",
    "Execute URLs": "目标 URL 访问与执行规则",
    "Allow/deny agent browser actuation access to specific URLs.": "配置允许或禁止智能体在自动化浏览器中打开和操控的特定 URL 规则。",
    "Organization Allowed Domains (Read-Only)": "企业组织下发的受信任域名列表（只读）",
    "These allowed domain rules are set by your organization": "此受信任域名白名单由所属企业或组织管理员策略统一下发，本地不可更改。",
    "Auto-Open Edited Files": "自动在编辑器中打开修改的文件",
    "Open files in the background if Agent creates or edits them": "当智能体新建或编辑修改代码文件时，自动在编辑器后台标签页中打开显示。",
    "Enable Shell Integration": "启用终端 Shell 深度集成",
    "Manage your plan, credentials, and general preferences.": "管理您的订阅计划、身份凭据与通用偏好设置。",
    "Your Plan:": "当前订阅方案：",
    "Project ID:": "Google Cloud 项目 ID：",
    "Enable Telemetry": "启用匿名使用统计与诊断遥测",
    "Marketing Emails": "接收新特性动态与产品资讯邮件",
    "Managed by Enterprise Gateway": "由企业统一认证网关托管",
    "Authenticated via Enterprise Gateway": "已通过企业级统一网关完成身份认证",
    "Project Required": "需要绑定 Cloud 项目",
    "License Required": "需要有效商业授权许可",
    "A Google Cloud project is required to use Antigravity.": "必须关联已启用相关 API 的 Google Cloud 项目方可使用 Antigravity。",
    "Model Quota": "模型调用速率与配额用量",
    "No quota information available.": "暂无可用配额信息或无法连接配额服务。",
    "Models & Usage": "模型配置与使用量概览",
    "Refresh quota and credits data": "刷新模型配额与抵扣点数数据",
    "Model Credits": "AI 积分抵扣额度 (Credits)",
    "Enable AI Credit Overages": "允许 AI 积分额度超额抵扣 (Overages)",
    "Custom Models": "自定义大模型列表",
    "Add Custom Model": "添加自定义模型",
    "Edit Custom Model": "编辑自定义模型配置",
    "Override feature environment": "替换运行环境特征参数 (Feature Gating)",
    "Host Environment": "宿主客户端环境",
    "Simulate running on a different OS.": "模拟在不同的操作系统上运行。",
    "Manage Skills": "管理智能体技能 (Skills)",
    "Customize Global Skills": "配置全局自定义技能",
    "Search MCP servers by name": "按服务名称搜索 MCP 服务器",
    "Default Customizations": "系统预置默认定制项",
    "Include Jetski Default Customizations": "加载官方预设默认技能与工具集",
    "Main Agent": "主智能体",
    "Select Agent": "选择智能体",
    "Getting scripts...": "正在获取智能体脚本...",
    "New Agent Script": "新建智能体脚本",
    "Agent options": "智能体选项",
    "Agent Script Command Configuration": "智能体脚本命令配置",
    "Command": "命令模式",
    "Python Script": "Python 脚本",
    "Python Coming Soon": "Python 脚本支持即将推出",
    "Command Spec (JSON) ": "命令执行规范 (JSON) ",
    "Overwrite if exists": "已存在时直接覆盖",
    "Select a workspace to save agent scripts to your workspace.": "选择一个工作区以将智能体脚本保存至该工作区。",
    "Browser Task": "浏览器任务",
    "Loading subagent...": "正在加载子智能体...",
    "Subagent ended": "子智能体已结束",
    "Subagent Conversation": "子智能体会话",
    "Search tabs, files, subagents, artifacts, tasks...": "搜索标签页、文件、子智能体、产物与任务...",
    "Failed to skip browser subagent:": "跳过浏览器子智能体失败:",
    "Failed to parse Subagents arg JSON:": "解析子智能体参数 JSON 失败:",
    "Configure the browser subagent. It requires": "配置浏览器子智能体。该功能需要",
    "Open full view": "展开完整视图",
    "Requested changes": "已请求修改",
    "Artifact image": "产物图片",
    "Artifact URL not found": "未找到产物 URL",
    "Artifact not found": "未找到产物",
    "Cannot revert messages with artifact comments": "无法撤回包含产物批注的消息",
    "Error fetching artifact": "获取产物失败",
    "Error: No artifact URI provided in URL parameters.": "错误：URL 参数中未提供产物 URI。",
    "Failed to get ARI for artifact:": "获取产物 ARI 标识失败:",
    "Failed to proceed": "继续执行失败",
    "Global Artifact": "全局产物",
    "No step index found for artifact code action step": "未找到产物代码操作步骤的对应索引",
    "Context Window": "上下文窗口",
    "Loading token usage...": "正在加载 Token 用量...",
    "Close split view and go to forked conversation": "关闭分栏视图并跳转至派生对话",
    "Parent conversation ID is required to fork.": "派生分支对话需要提供父级对话 ID。",
    "Failed to fork conversation": "派生对话失败",
    "Share Conversation (Preview)": "共享对话 (预览版)",
    "Importing shared conversation...": "正在导入共享对话...",
    "Failed to copy conversation markdown:": "复制对话 Markdown 文本失败:",
    "Search conversations (by name or Cascade ID)": "搜索对话 (按名称或 Cascade ID)",
    "Search for conversations...": "搜索对话...",
    "Search conversations...": "搜索历史对话...",
    "Audio streaming transcription not available": "音频流式转录服务不可用",
    "Streaming audio feature is not available": "流式音频功能当前不可用",
    "Thinking for ": "正在深度思考 ",
    "• Thinking duration:": "• 思考耗时:",
    "Active token streaming duration from the language model.": "大语言模型活跃 Token 流式生成耗时。",
    "Error while running MCP tool": "执行 MCP 工具时发生错误",
    "Configure MCP Server": "配置 MCP 服务器",
    "Loading MCP servers...": "正在加载 MCP 服务器列表...",
    "Search for MCP servers to add to your configuration": "搜索要添加到配置中的 MCP 服务器",
    "No MCP servers yet. Use Add to browse the store.": "暂无 MCP 服务器。请点击“添加”浏览商店安装。",
    "Modify permissions for file, terminal, and MCP tools.": "配置文件、终端与 MCP 工具的执行权限。",
    "Could not determine MCP config file path": "无法确定 MCP 配置文件存储路径",
    "Could not read MCP config file": "无法读取 MCP 配置文件",
    "Failed to open MCP config file": "打开 MCP 配置文件失败",
    "No method available to open MCP config file": "未找到可用于打开 MCP 配置文件的关联应用程序",
    "Delete MCP Server": "删除 MCP 服务器",
    "Refresh MCP servers": "刷新 MCP 服务器状态",
    "MCP Authentication Error:": "MCP 身份验证错误:",
    "MCP Configuration Error:": "MCP 配置格式错误:",
    "MCP Install Error:": "MCP 安装失败错误:",
    "MCP Servers Disabled": "MCP 服务器已被禁用",
    "No MCP servers found for ": "未找到匹配的 MCP 服务器: ",
    ...
    "Fill": "撑满 / 自适应宽度",
        ...
        "Files modified by the agent in this conversation": "智能体在本轮对话中修改的文件",
        "Cannot revert messages with file diff comments": "包含文件差异批注的消息无法撤销",
        "The background color of unchanged blocks in the diff editor.": "Diff 编辑器中未变更代码块的背景颜色。",
        "The foreground color of unchanged blocks in the diff editor.": "Diff 编辑器中未变更代码块的前景颜色。",
        "Diff overview ruler foreground for inserted content.": "Diff 概览标尺中新增内容的前景色。",
        "Diff overview ruler foreground for removed content.": "Diff 概览标尺中删除内容的前景色。",
        "Diff worker fatal error:": "Diff 后台工作线程发生致命错误：",
        "Toggle Diff": "切换差异视图",
        ...
        "Process exited with an error": "进程异常退出",
        "Error while running command": "执行命令时发生错误",
        "Cannot start best-of-N mode: background tasks are still running.": "无法启动 Best-of-N 模式：仍有后台任务在运行。",
        "Process completed with exit code": "进程执行完毕，退出代码：",
        "Start Snapshot": "起始快照",
        "End Snapshot": "完成状态快照",
        "Select the latest snapshot from the": "从列表中选择最新的工作区快照：",
        "Staged index changes and working tree changes": "Git 暂存区更改与工作区未暂存更改",
        "Time spent deciding a checkpoint was needed and generating the context summary.": "用于评估检查点触发条件及生成上下文摘要所耗费的时间。",
        "Discard Workspace Changes": "放弃工作区更改",
        "Enable Sandbox Mode": "启用沙箱模式",
        "Enable Sandbox Mode (Preview)": "启用沙箱模式 (预览版)",
        "Enable Terminal Sandbox": "启用终端沙箱隔离",
        ...
        "Commands the agent can run outside the sandbox.": "允许智能体在沙箱外部运行的命令白名单。",
        "Configure allowed commands outside the sandbox.": "配置允许在沙箱外部执行的命令白名单。",
        "The Agent attempted to interact with some sites that are not allowlisted": "智能体尝试访问未列入白名单的外部站点",
        "Failed to add URL to allowlist:": "将网址添加至白名单失败：",
        "Failed to add URL to allowlist: addToBrowserAllowlist is undefined": "将网址添加至白名单失败：addToBrowserAllowlist 未定义",
        "These MCP allowlist rules are enforced by your organization's administrator.": "这些 MCP 白名单规则由您所在组织的管理员统一强制执行。",
        "Upstream proxy returned an empty response": "上游代理服务器返回了空响应",
        "Allow full browser script execution without prompting.": "允许全量执行浏览器脚本且无需提示确认。",
        "Block all browser JavaScript execution.": "禁止所有浏览器端 JavaScript 脚本执行。",
        "Browser Javascript Execution Policy": "浏览器端 JavaScript 执行策略",
        "The agent asks for permission before executing commands matched by a deny list entry.": "当执行命中黑名单规则的命令时，智能体将请求用户明确授权。",
        "Application": "应用偏好设置",
        "Skin": "界面形态与风格",
        "Tab": "Tab 智能补全与导航",
        "Browser": "内置浏览器与执行规则",
        ...
        "Choose the product skin that fits how you work.": "选择契合您工作习惯的产品交互界面形态。",
        "Workspace settings have moved": "工作区设置已迁移",
        "Workspace settings have moved to the Advanced section of General settings.": "工作区相关设置已整合迁移至【通用设置】的【高级】分区中。",
        "Best of N settings have moved": "多候选探索设置已迁移",
        "Best of N settings have moved to the Advanced section of General settings.": "多候选探索 (Best of N) 设置已整合迁移至【通用设置】的【高级】分区中。",
        "Browser settings have moved": "浏览器设置已迁移",
        "Browser settings have moved to the Browser section of General settings.": "浏览器设置已整合迁移至【通用设置】的【浏览器】分区中。",
        "Labs settings have moved": "实验特性设置已迁移",
        "Labs settings have moved to the Advanced section of General settings.": "实验特性 (Labs) 设置已整合迁移至【通用设置】的【高级】分区中。",
        "Change VCS in General settings, under Advanced": "请在【通用设置】的【高级】选项中切换版本控制系统 (VCS)",
        "Create a new Fig workspace from your initial prompt. Customize in General Settings, under Advanced.": "根据您的初始提示词创建全新的 Fig 工作区。可在【通用设置】的【高级】选项中自定义。",
        "Create a new Piper (g4) workspace from your initial prompt. Customize in General Settings, under Advanced.": "根据您的初始提示词创建全新的 Piper (g4) 工作区。可在【通用设置】的【高级】选项中自定义。",
        "Create a new JJ workspace from your initial prompt. Customize in General Settings, under Advanced.": "根据您的初始提示词创建全新的 Jujutsu (JJ) 工作区。可在【通用设置】的【高级】选项中自定义。",
        "Create a new Cog (GoB) workspace from your initial prompt. Customize in General Settings, under Advanced.": "根据您的初始提示词创建全新的 Cog (GoB) 工作区。可在【通用设置】的【高级】选项中自定义。",
        "Use full Git clones for Best of N": "在多候选探索中对 Git 仓库使用完整独立克隆",
        "Use full VCS clones for Best of N": "在多候选探索中对 VCS 仓库使用完整独立克隆",
        "Security Preset": "安全策略预设",
        "Full access": "完全访问模式 (整机与网络权限开放)",
        ...
        "Global Permissions": "全局安全权限",
        "Review Permissions": "审核并授予权限",
        "Inherits your Global Permissions when working in this project.": "在当前项目中执行时自动继承全局安全权限配置。",
        "automation. Automations do not inherit Global Permissions.": "自动化运行任务不会继承全局权限，需单独配置安全授权。",
        "Agents have full access to your machine and external resources.": "智能体拥有对整机系统与外部网络资源的不受限访问权限，适合完全信任的高效场景。",
        "Agents run in a secure sandbox that restricts access to external resources outside of your trusted folders.": "智能体在安全隔离沙箱中执行，严格限制访问受信任工作区以外的外部系统资源与网络。",
        "Browser Actuation Permissions": "浏览器自动化操作与页面控制权限",
        "Execute URLs": "目标 URL 访问与执行规则",
        "Allow/deny agent browser actuation access to specific URLs.": "配置允许或禁止智能体在自动化浏览器中打开和操控的特定 URL 规则。",
        "Organization Allowed Domains (Read-Only)": "企业组织下发的受信任域名列表（只读）",
        "These allowed domain rules are set by your organization": "此受信任域名白名单由所属企业或组织管理员策略统一下发，本地不可更改。",
        "Allows the agent to access files outside of your current workspace.": "允许智能体读取和修改当前工作区根目录之外的本地文件。",
        "Auto-Open Edited Files": "自动在编辑器中打开修改的文件",
        "Open files in the background if Agent creates or edits them": "当智能体新建或编辑修改代码文件时，自动在编辑器后台标签页中打开显示。",
        "Enable Shell Integration": "启用终端 Shell 深度集成",
        "Controls whether terminal commands require your approval before running.": "控制终端命令在后台执行前是否需要人工弹窗审批确认。",
        "When enabled, the agent will be able to access past conversations to inform its responses.": "启用后，智能体将能够检索历史对话上下文，提供更具连贯性的回答。",
        "Manage your plan, credentials, and general preferences.": "管理您的订阅计划、身份凭据与通用偏好设置。",
        "Your Plan:": "当前订阅方案：",
        "Project ID:": "Google Cloud 项目 ID：",
        "Enable Telemetry": "启用匿名使用统计与诊断遥测",
        ...
        "Add Custom Model": "添加自定义模型",
        "Edit Custom Model": "编辑自定义模型配置",
        "Developer-only tools. These settings are stored locally in this browser and do not affect other users.": "仅供开发者使用的调试工具。这些设置仅保存在本地当前浏览器环境中，不会影响其他用户。",
        "Override feature environment": "替换运行环境特征参数 (Feature Gating)",
        "When enabled, the values below replace the auto-detected OS, host, and user type used for feature gating.": "启用后，以下配置将覆盖自动探测的操作系统、宿主环境与用户类型，以测试不同的功能特性开关。",
        "Host Environment": "宿主客户端环境",
        "Simulate running on a different OS.": "模拟在不同的操作系统上运行。",
        "Simulate a different host (Electron, desktop or mobile Web, Extension, remote control, or installed PWA).": "模拟不同的宿主客户端形态（Electron 客户端、桌面/移动 Web、IDE 插件、远程受控或 PWA 应用）。",
        "Manage Skills": "管理智能体技能 (Skills)",
        "Customize Global Skills": "配置全局自定义技能",
        "Search MCP servers by name": "按服务名称搜索 MCP 服务器",
        "Default Customizations": "系统预置默认定制项",
        "Include Jetski Default Customizations": "加载官方预设默认技能与工具集",
        "When enabled, the agent will include default customizations, including default skills.": "启用后，智能体将自动加载官方预设的开箱即用技能与工具集。",
        "To modify editor settings, open Settings within the editor window.": "如需修改编辑器基础核心参数（如字体、缩进、微型地图等），请直接在编辑器主窗口中打开设置。",
        "Main Agent": "主智能体",
        "Select Agent": "选择智能体",
        "Getting scripts...": "正在获取智能体脚本...",
        "New Agent Script": "新建智能体脚本",
        ...
        "Overwrite if exists": "已存在时直接覆盖",
        "Select a workspace to save agent scripts to your workspace.": "选择一个工作区以将智能体脚本保存至该工作区。",
        "This script can be version controlled and shared with your team.": "此脚本可纳入版本控制系统，方便与您的团队协同共享。",
        "Browser Task": "浏览器任务",
        "Loading subagent...": "正在加载子智能体...",
        "Subagent ended": "子智能体已结束",
        "Subagent Conversation": "子智能体会话",
        "Cannot start best-of-N mode: some subagents are still running.": "无法启动 Best-of-N 候选对比模式：部分子智能体仍在运行中。",
        "Search tabs, files, subagents, artifacts, tasks...": "搜索标签页、文件、子智能体、产物与任务...",
        "The browser subagent can be invoked by typing /browser in the conversation input box.": "可在对话输入框中输入 /browser 快速调用浏览器子智能体。",
        "Failed to skip browser subagent:": "跳过浏览器子智能体失败:",
        "Failed to parse Subagents arg JSON:": "解析子智能体参数 JSON 失败:",
        "Configure the browser subagent. It requires": "配置浏览器子智能体。该功能需要",
        "Open full view": "展开完整视图",
        ...
        "Context Window": "上下文窗口",
        "Loading token usage...": "正在加载 Token 用量...",
        "This conversation grew too large, so the history before this point may be missing or incomplete.": "此对话上下文已超出容量上限，在此节点之前的历史记录可能已被截断或不完整。",
        "This step was excluded from the messages sent to the model and is hidden in the conversation view.": "此步骤已从发送给模型的上下文消息中剔除，并在对话视图中处于隐藏状态。",
        "This step was sent to the model, but it is hidden in the standard conversation view.": "此步骤已提交给模型处理，但在标准对话视图中被收起隐藏。",
        "You can prompt the model to try again or start a new conversation if the error persists.": "您可以提示模型重试；如果错误依然发生，建议新建对话重试。",
        "You have a conversation in progress. Updating will cancel it and you'll need to manually restart it.": "当前有正在执行中的对话。执行更新将中断此对话，稍后需手动重新发起。",
        "Rewinds the conversation only. Files in this workspace stay as they are now.": "仅回退对话上下文记录。工作区中的文件将保持现状不变。",
        "Close split view and go to forked conversation": "关闭分栏视图并跳转至派生对话",
        "Parent conversation ID is required to fork.": "派生分支对话需要提供父级对话 ID。",
        "Failed to fork conversation": "派生对话失败",
        "Share Conversation (Preview)": "共享对话 (预览版)",
        ...
        "Search for conversations...": "搜索对话...",
        "Search conversations...": "搜索历史对话...",
        "Are you sure you want to delete this conversation? This action cannot be undone.": "确定要彻底删除此对话吗？此操作不可恢复。",
        "Are you sure you want to mark 1 conversation as read? This action cannot be undone.": "确定要将此对话标记为已读吗？此操作不可恢复。",
        "Audio streaming transcription not available": "音频流式转录服务不可用",
        "Streaming audio feature is not available": "流式音频功能当前不可用",
        "Thinking for ": "正在深度思考 ",
        "• Thinking duration:": "• 思考耗时:",
        ...
        "Error while running MCP tool": "执行 MCP 工具时发生错误",
        "Configure MCP Server": "配置 MCP 服务器",
        "Enter the required configuration values to install this MCP server.": "请输入安装此 MCP 服务器所需的配置参数。",
        "Loading MCP servers...": "正在加载 MCP 服务器列表...",
        "Search for MCP servers to add to your configuration": "搜索要添加到配置中的 MCP 服务器",
        "No MCP servers yet. Use Add to browse the store.": "暂无 MCP 服务器。请点击“添加”浏览商店安装。",
        "Your administrator has disabled MCP servers for this workspace.": "管理员已在当前工作区禁用了 MCP 服务器功能。",
        "Modify permissions for file, terminal, and MCP tools.": "配置文件、终端与 MCP 工具的执行权限。",
        "Could not determine MCP config file path": "无法确定 MCP 配置文件存储路径",
        "Could not determine MCP config file path. Please check your home directory settings.": "无法确定 MCP 配置文件存储路径，请检查您的用户主目录 (Home) 配置。",
        "Could not read MCP config file": "无法读取 MCP 配置文件",
        "Failed to open MCP config file": "打开 MCP 配置文件失败",
        "No method available to open MCP config file": "未找到可用于打开 MCP 配置文件的关联应用程序",
        "Delete MCP Server": "删除 MCP 服务器",
        ...
        "No MCP servers found for ": "未找到匹配的 MCP 服务器: ",
        "No MCP servers installed": "未安装任何 MCP 服务器",
        # --- 2.17.0 智能体与工作流审计新增词条 ---
        '"(subagent)"': '"(子智能体)"',
        '"Child Subagent"': '"子级智能体"',
        '"Child Subagents"': '"子级智能体列表"',
        '"Loading subagent..."': '"正在加载子智能体..."',
        '"Search tabs, files, subagents, artifacts, tasks..."': '"搜索标签页、文件、子智能体、产物与任务..."',
        '"Subagent Conversation"': '"子智能体对话"',
        '"Subagent ended"': '"子智能体已结束"',
        '"The browser subagent has been disabled by your administrator. Contact your administrator to enable browser use."': '"管理员已禁用浏览器子智能体。请联系您的管理员以启用浏览器功能。"',
        '"Context Window"': '"上下文窗口"',
        '"Loading token usage..."': '"正在加载 Token 用量..."',
        '"Audio streaming transcription not available"': '"音频流式转录不可用"',
        '"Streaming audio feature is not available"': '"流式音频功能不可用"',
        '"Time spent by the language model generating and streaming the tool call."': '"大语言模型生成并流式传输工具调用的耗时。"',
        '"Plan Review Policy"': '"规划评审策略"',
        '"Thought for "': '"思考耗时 "',
        '"Used MCP tool: "': '"已调用 MCP 工具: "',
    }
    ...
    for original, translated in translations.items():
        content, replaced = apply_string_translation(content, original, translated)
        if replaced:
            replaced_count += 1

    print(f"Replaced {replaced_count} of {len(translations)} strings safely.")
...
(function() {
  ...
  const translateDynamicRegex = (text) => {
    ...
    // 子智能体带参调用与归属
    if (/^Invoked subagent:\s*(.+)$/.test(t)) {
      return text.replace(/^Invoked subagent:\s*(.+)$/, "已调用子智能体: $1");
    }
    if (/^subagent of "(.+)"$/.test(t)) {
      return text.replace(/^subagent of "(.+)"$/, "隶属于 \"$1\" 的子智能体");
    }
    // 思考中多节点拼接兜底
    if (/^正在深度思考\s*(\d+)\s*s$/.test(t)) {
      return text.replace(/^正在深度思考\s*(\d+)\s*s$/, "正在深度思考 ($1 秒)...");
    }
    // MCP 服务器配置动态标题
    if (/^The (.+) MCP server requires configuration parameters before it can be started\.$/.test(t)) {
      return text.replace(/^The (.+) MCP server requires configuration parameters before it can be started\.$/, "MCP 服务器“$1”需要先完善配置参数才能启动。");
    }
    if (/^Configure (.+)$/.test(t) && !t.includes("Settings") && !t.includes("the")) {
      return text.replace(/^Configure (.+)$/, "配置 $1");
    }
    // 对比度过低警告动态匹配
    if (/^Low contrast ratio \(([\d.]+):1\)\. Text may be hard to read\.$/.test(t)) {
      return text.replace(/^Low contrast ratio \(([\d.]+):1\)\. Text may be hard to read\.$/, "色彩对比度过低 ($1:1)，文字可能难以辨识。");
    }
    return text;
  };
